...
class Task:
            ...
            if self.mode == "sc":
                with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    futures = [executor.submit(reasoner.generate_response, 
                        prompt, temperature=0.7) for _ in range(5)]
    ...
    def evaluate(self, data_path):
        ...
        for item in tqdm(data, desc="Evaluating"):
            ...
            # for base, cot
                ...
                if self.mode == "srr" or self.mode == "sr" or self.mode == "hr":
    ...
    def run_srr(self, config_path, output_path, model, position=None):
        with open(config_path, "r") as f:
            config = json.load(f)
        python_executor = PythonExecutor()
        workers_setteing = {0: 10, 1: 40, 2: 40}
        # add reload function
    ...
    def eval_srr(self, data_path, round=0):
        ...
        for item in tqdm(data, desc=f"Evaluating SRR responses in round {round}"):
            ...
            if round == max_round and "selected_response" in item:
    ...
    def run_hr(self, config_path, output_path, model, position=None):
            ...
            if round_index == 0:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    ...
                    futures = [executor.submit(
                        reasoner.generate_response, 
                        prompt, 
                        temperature=0.7) for _ in range(3)]
                    responses = [f.result() for f in concurrent.futures.as_completed(futures)]
                item["response"].extend([
                    {"round": round_index, "response": response, "correct": [], "wrong": []} for response in responses
                ])
            else:
                last_response = item["response_candidates"][-1]["rule"]
                last_wrong = item["response_candidates"][-1]["wrong"]
                if len(last_wrong) != 0:
                    wrong_examples = "\n".join([f"Input: {ex['input']}    Expected output: {ex['output']}    Your output: {ex['res']}" for ex in last_wrong])
                else:
                    wrong_examples = "No wrong examples."
                prompt = self.prompt["hr_iterative"].format(rule=last_response, wrong_examples=wrong_examples, examples="\n".join([f"Input: {ex['input']}\nOutput: {ex['output']}" for ex in item["examples"]]))
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    futures = [executor.submit(
                        reasoner.generate_response, 
                        prompt, 
        ...
        for round_index in range(last_round, self.refine_round):
            ...
            for item in tqdm(processed_items, desc=f"Evaluating responses for round {round_index}"):
                ...
                for resp in item["response"]:
                    ...
                    for exe_res in resp["execution"]:
                        if self.check_correctness(exe_res["res"], exe_res["output"]):
                            ...
                            resp["wrong"].append(exe_res)
                    resp["accuracy"] = len(resp["correct"]) / len(item["examples"])
                # select the best response in this round, i.e., from the last 3 responses
                
                best_response = max(item["response"][-3:], key=lambda x: x["accuracy"])
                # if the accuracy is larger than 0.9, stop
                if best_response["accuracy"] >= 0.9:
                    item["selected_response"] = best_response["response"]
                else:
                ...
                # if this is the last round, select the best response
                if round_index == self.refine_round - 1:
                    item["selected_response"] = max(item["response"][-3:], key=lambda x: x["accuracy"])["response"]
